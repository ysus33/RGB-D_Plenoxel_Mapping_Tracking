...
class SCANDataset(DatasetBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Load scene config
        scene_name = root.split("/")[-1][5:9] + ".json"
        script_dir = os.path.dirname(__file__)
        scene_config_path = os.path.join(script_dir, "../configs/scannet")
        ...
        fx, fy, cx, cy = self.loadIntrinParam(scene_config)
        ...
        data_path = root
        pose_path = os.path.join(root, "gt.txt")

        print("LOAD DATA", data_path)
        img = sorted(glob.glob(f'{data_path}/rgb/frame-*.color.jpg'))
        depth = sorted(glob.glob(f'{data_path}/depth/frame-*.depth.pgm'))
        ...
        for k in tqdm(range(start, len(img), 10)):
            ...
            d = depth[k]
            p = pose[k]
            c2w = np.array(list(map(float, p.split()))).reshape(3, 4)
            c2w = np.vstack((c2w, np.array([0,0,0,1])))
            c2w = torch.from_numpy(c2w).float()
            im_gt = imageio.imread(i)
            im_gt = cv2.resize(im_gt, (640, 480))
